...
@section('content-header')
    <ol class="breadcrumb text-uppercase font-size-xs p-0">
        ...
        <li class="breadcrumb-item"><a href="{{ route('account') }}">@lang('strings.account')</a></li>
        <li class="breadcrumb-item active" aria-current="page">@lang('strings.security')</li>
    </ol>
    <h5 class="display-4 mt-1 mb-2 font-weight-bold">@lang('base.security.header')</h5>
    <p class="text-black-50 mb-0">@lang('base.security.header_sub')</p>
@endsection
            ...
            <div class="card card-box">
                <div class="card-header">
                    <div class="card-header--title">
                        <h5 class="my-3">@lang('base.security.sessions')</h5>
                    </div>
                </div>
                @if(!is_null($sessions))
                                    ...
                                    <td>
                                        <a href="{{ route('account.security.revoke', $session->id) }}">
                                            <button class="btn btn-xs btn-danger"><i
                                                    class="fa fa-trash-o"></i> @lang('strings.revoke')</button>
                                        </a>
                                    </td>
                                </tr>
                            @endforeach
                ...
                @else
                    <div class="card-body">
                        <p class="text-warning">@lang('base.security.session_mgmt_disabled')</p>
                    </div>
                @endif
            </div>
        </div>
        <div class="col-md-6">
            <div class="card card-box {{ (Auth::user()->use_totp) ? 'box-success' : 'box-danger' }}">
                <div class="card-header">
                    <div class="card-header--title">
                        <h5 class="my-3">@lang('base.security.2fa_header')</h5>
                    </div>
                </div>
                @if(Auth::user()->use_totp)
                    <form action="{{ route('account.security.totp') }}" method="post">
                        ...
                        <div class="card-footer">
                            ...
                            {{ method_field('DELETE') }}
                            <button type="submit"
                                    class="btn btn-danger btn-sm">@lang('base.security.disable_2fa')</button>
                        </div>
                    </form>
                @else
                    <form action="#" method="post" id="do_2fa">
                        ...
                        <div class="card-footer">
                            {!! csrf_field() !!}
                            <button type="submit"
                                    class="btn btn-success btn-sm">@lang('base.security.enable_2fa')</button>
                        </div>
                    </form>
                @endif
            </div>
                ...
                <form action="#" method="post" id="2fa_token_verify">
                    ...
                    <div class="modal-body" id="modal_insert_content">
                        ...
                        <div class="row">
                            <div class="col-md-6 text-center">
                                <span id="hide_img_load"><i class="fa fa-spinner fa-spin"></i> Loading QR Code...</span><img
                                    src="" id="qr_image_insert" style="display:none;" />
                            </div>
                            <div class="col-md-6">
                    ...
                    <div class="modal-footer">
                        <button type="submit" class="btn btn-primary btn-sm"
                                id="submit_action">@lang('strings.submit')</button>
                        <button type="button" class="btn btn-danger btn-sm" data-dismiss="modal"
                                id="close_reload">@lang('strings.close')</button>
                    </div>
                </form>
            </div>
        </div>
