...
<div class="card-header with-border">
    <div class="card-header--title">
        <h5 class="my-3">/home/container{{ $directory['header'] }}</h5>
    </div>
    ...
    <div class="card-header--actions">
        ...
        <label class="btn btn-primary btn-icon btn-sm" style="margin-top: .5rem;">
            Upload <i class="fa fa-fw fa-upload"></i><input type="file" id="files_touch_target" style="display: none;">
        </label>
        <div class="btn-group hidden-xs">
            <button type="button" id="mass_actions" class="btn btn-sm btn-info dropdown-toggle disabled"
                    data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                @lang('server.files.mass_actions') <span class="caret"></span>
            </button>
            <ul class="dropdown-menu dropdown-massactions text-center">
                <li><a href="#" id="selective-deletion" data-action="selective-deletion">@lang('server.files.delete') <i
                            class="fa fa-fw fa-trash-o"></i></a></li>
            </ul>
        </div>
    </div>
</div>
    ...
    <table class="table table-hover" id="file_listing" data-current-dir="{{ rtrim($directory['header'], '/') . '/' }}">
        ...
        <tr>
            <th class="middle min-size">
                <input type="checkbox" class="select-all-files hidden-xs mr-2" data-action="selectAll"><i
                    class="fa fa-refresh muted muted-hover use-pointer" data-action="reload-files"
        ...
        @if (isset($directory['show']) && $directory['show'] === true)
            <tr data-type="disabled">
                ...
                <td data-name="{{ rawurlencode($directory['link']) }}">
                    <a href="/server/{{ $server->uuidShort }}/files"
                       data-action="directory-view">&larr; {{ $directory['link_show'] }}</a>
                </td>
                <td class="hidden-xs"></td>
                <td class="hidden-xs"></td>
                <td></td>
        ...
        @foreach ($folders as $folder)
            <tr data-type="folder">
                <td class="middle min-size" data-identifier="type">
                    <input type="checkbox" class="select-folder hidden-xs mr-2" data-action="addSelection"><i
                        class="fa fa-folder" style="margin-left: 0.859px;"></i>
                </td>
                <td data-identifier="name" data-name="{{ rawurlencode($folder['entry']) }}"
                    data-path="@if($folder['directory'] !== ''){{ rawurlencode($folder['directory']) }}@endif/">
                    <a href="/server/{{ $server->uuidShort }}/files"
                       data-action="directory-view">{{ $folder['entry'] }}</a>
                </td>
                ...
                <td class="min-size">
                    <button class="btn btn-xxs btn-default disable-menu-hide" data-action="toggleMenu"
                            style="padding:2px 6px 0px;"><i class="fa fa-ellipsis-h disable-menu-hide"></i></button>
                </td>
            </tr>
        @endforeach
        @foreach ($files as $file)
            <tr data-type="file" data-mime="{{ $file['mime'] }}">
                <td class="middle min-size" data-identifier="type"><input type="checkbox"
                                                                          class="select-file hidden-xs mr-2"
                ...
                <td data-identifier="name" data-name="{{ rawurlencode($file['entry']) }}"
                    data-path="@if($file['directory'] !== ''){{ rawurlencode($file['directory']) }}@endif/">
                    @if(in_array($file['mime'], $editableMime))
                        @can('edit-files', $server)
                            <a href="/server/{{ $server->uuidShort }}/files/edit/@if($file['directory'] !== ''){{ rawurlencode($file['directory']) }}/@endif{{ rawurlencode($file['entry']) }}"
                               class="edit_file">{{ $file['entry'] }}</a>
                        @else
                            {{ $file['entry'] }}
                        @endcan
                    @else
                ...
                <td class="min-size settings-menu">
                    <button class="btn btn-xxs btn-default disable-menu-hide" data-action="toggleMenu"
                            style="padding:2px 6px 0px;"><i class="fa fa-ellipsis-h disable-menu-hide"></i></button>
                </td>
            </tr>
...
<div class="modal" tabindex="-1" role="dialog" id="actionsModal">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body text-center" id="actionsModalBody">
