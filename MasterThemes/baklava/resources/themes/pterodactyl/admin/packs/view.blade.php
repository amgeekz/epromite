...
<form action="{{ route('admin.packs.view', $pack->id) }}" method="POST">
    <div class="row">
        <div class="col-md-6">
            <div class="card card-primary">
                <div class="card-header with-border">
                    <h3 class="card-title">Pack Details</h3>
                </div>
                <div class="card-body">
        ...
        <div class="col-md-6">
            <div class="card card-primary">
                <div class="card-header with-border">
                    <h3 class="card-title">Pack Configuration</h3>
                </div>
                <div class="card-body">
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <p class="text-muted small">Check this card if user should be able to select this pack to install on their servers.</p>
                    </div>
                    <div class="form-group">
                        <div class="checkcard checkcard-primary no-margin-bottom">
                        ...
                        <p class="text-muted small">Check this card if this pack is visible in the dropdown menu. If this pack is assigned to a server it will be visible regardless of this setting.</p>
                    </div>
                    <div class="form-group">
                        <div class="checkcard checkcard-warning no-margin-bottom">
                            <input id="pLocked" name="locked" type="checkbox" value="1" {{ ! $pack->locked ?: 'checked' }}/>
                            <label for="pLocked">
                                Locked
                            </label>
                        </div>
                        <p class="text-muted small">Check this card if servers assigned this pack should not be able to switch to a different pack.</p>
                    </div>
                </div>
                <div class="card-footer with-border">
                    {!! csrf_field() !!}
                    <button name="_method" value="PATCH" class="btn btn-md btn-primary pull-right" type="submit">Save</button>
                    <button name="_method" value="DELETE" class="btn btn-md btn-danger pull-left muted muted-hover" type="submit"><i class="feather icon-trash"></i></button>
                </div>
            </div>
        </div>
    </div>
...
<div class="row">
    <div class="col-md-12">
        <div class="card">
            <div class="card-header with-border">
                <h3 class="card-title">Servers Using This Pack</h3>
            </div>
...
<div class="row">
    <div class="col-md-12">
        <form action="{{ route('admin.packs.view.export', $pack->id) }}" method="POST">
            {!! csrf_field() !!}
            <button type="submit" class="btn btn-md btn-success pull-right">Export</button>
        </form>
        <form action="{{ route('admin.packs.view.export', ['id' => $pack->id, 'files' => 'with-files']) }}" method="POST">
            {!! csrf_field() !!}
            <button type="submit" class="btn btn-md btn-secondary pull-right muted muted-hover" style="margin-right:10px;">Export with Archive</button>
        </form>
    </div>
</div>
@endsection
